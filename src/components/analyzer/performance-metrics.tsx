...
export default function PerformanceMetrics({ performance }: PerformanceMetricsProps) {
  const metrics = [
    { icon: DollarSign, label: "TOTAL VOLUME", value: `${performance.total_volume_sol.toFixed(3)} SOL`, color: "text-foreground" },
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.map((metric, index) => (
          <div key={index} className="flex items-start gap-3">
            <div className="w-8 h-8 bg-foreground/10 flex items-center justify-center flex-shrink-0" style={{ borderRadius: 0 }}>
