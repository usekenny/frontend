          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                } flex items-center justify-center text-white font-bold text-xs title-font`} style={{ borderRadius: 0 }}>
                  {token.symbol.slice(0, 2)}
                </div>
                <div>
                  <h4 className="text-foreground font-bold text-lg">{token.symbol}</h4>
                  <p className="text-foreground/40 text-xs font-mono">{token.token_address}...{token.token_address.slice(-6)}</p>
                </div>
              </div>

              {/* Status badges */}
              <div className="flex flex-col items-end gap-1">
                <div className={`px-2 py-1 text-xs font-bold title-font ${
                  token.status === "sold" ? "bg-foreground/10 text-foreground" : "bg-[#FF6B00]/10 text-[#FF6B00]"
                }`} style={{ borderRadius: 0 }}>
                  {token.status.toUpperCase()}
                </div>
                <div className={`px-2 py-1 text-xs font-bold title-font ${
                  token.profit_status === "profit" ? "bg-[#14F195]/10 text-[#14F195]" : "bg-[#FF223B]/10 text-[#FF223B]"
                }`} style={{ borderRadius: 0 }}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-3 border-t border-foreground/10">
              ...
              <div>
                ...
                <p className={`font-bold text-sm title-font ${token.pnl_sol >= 0 ? "text-[#14F195]" : "text-[#FF223B]"}`}>
