...
export default function AgentPanel() {
	...
	// Chat hook
	const { messages, isLoading, isAgentThinking, sendMessage, error: chatError, animatedMessageId } = useElizaChat({
		agentId: agent?.id,
	});
	...
	const convertToToolPart = (message: Message): ToolPart => {
		...
		if (message.rawMessage) {
			try {
				const parsed =
					typeof message.rawMessage === 'string' ? JSON.parse(message.rawMessage) : message.rawMessage;
						...
						>
							...
							<div className="border-t border-foreground/10 p-4 bg-background">
								<div className="flex items-end gap-2">
									...
									>
										{isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5 text-foreground" />}
									</Button>
								</div>
